...
import { AuthState, changeRefreshToken, userLogout } from '../../redux/slice/authSlice';
...
const api = axios.create({
    baseURL: 'https://localhost:50948/api/v1/'
});
...
api.interceptors.request.use(
    (config: AxiosRequestConfig<any>): AxiosRequestConfig<any> => {
        ...
        const newHeader = { ...(config.headers || {}), ...authHeader() } as AxiosRequestHeaders;
